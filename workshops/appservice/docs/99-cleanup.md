...
All your workshop resources (App Service, Azure SQL, Log Analytics, Application Insights, and managed identity) live in a single resource group. Deleting the resource group deletes everything in one command.
...
This single command removes everything: the App Service Plan, the web app, the Azure SQL server and database, Log Analytics, Application Insights, the managed identity, and the SRE Agent (if deployed in the same resource group).
...
1. **Deployed realistic Azure infrastructure** using Bicep — an App Service Plan hosting a .NET 10 web app, an Azure SQL database with managed access controls, and comprehensive monitoring
2. **Deployed a PaaS application** to Azure App Service with secure, identity-based authentication to a backend database
...
4. **Simulated a real operational failure** — a seemingly innocent infrastructure change (removing a SQL grant) that broke your application
...
- **App Service docs:** [Azure App Service documentation](https://docs.microsoft.com/azure/app-service/)
