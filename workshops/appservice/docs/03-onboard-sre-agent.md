...
Create and configure an Azure SRE Agent that will monitor your App Service and respond to incidents.
...
- Your **App Service configuration** to understand how the app is deployed
...
When we intentionally break the app in Module 5 by removing the SQL grant from the Bicep code, the agent will identify that specific commit as the culprit.
...
- Query **Azure Monitor metrics** (CPU, memory, request rates, error rates)
- Read **Log Analytics logs** (app console output, HTTP logs, authentication failures)
- Check **App Service status** (running, stopped, restarting)
...
The agent will automatically explore your connected codebase and Azure resources. It reads your Bicep templates and GitHub workflows to build an initial picture of your setup. You'll see it ask clarifying questions like "What does this deployment do?" or "What's the relationship between these services?"

### Share Your Knowledge

When the agent asks, tell it about your workshop setup. For example:

> "We're running a demo App Service with a .NET 10 web app that connects to an Azure SQL database using managed identity. The app exposes `/`, `/health`, and `/products` endpoints. We care most about the health of the App Service and whether the app can successfully connect to Azure SQL for read operations."

Then share a debugging hint that will be invaluable later:

> "If the app can't connect to Azure SQL, the first thing to check is the managed identity SQL grant. The app authenticates using DefaultAzureCredential with a User-Assigned Managed Identity, which relies on a contained database user in Azure SQL granted `db_datareader`. If that user is missing or the grant is revoked, authentication will fail with a login error."
...
**Tip:** The more specific and actionable your onboarding information, the faster the agent will diagnose issues in Module 6. Your debugging hint about SQL grants will directly help when the agent investigates the fault we introduce in Module 5.
...
2. **Check the Azure Resources:** You should see the App Service resources in the **Monitor** → **Resource Mapping**
3. **Ask the agent:** In the onboarding conversation, ask "What Azure resources do you see in my resource group?" — it should list your App Service, Azure SQL server, Log Analytics workspace, and other resources you created in Module 1
4. **Ask the agent:** "Can you tell me about the app's architecture from the code?" — it should reference your Bicep templates and application code
