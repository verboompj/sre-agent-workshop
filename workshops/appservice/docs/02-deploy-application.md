...
4. Fill in the workflow inputs:
   - **workloadName** (default: `srelabapp`) — **must match what you used in Module 1**. If you deployed infrastructure with a different name, use the same name here.
...
WEB_APP=$(az webapp list --resource-group rg-srelabapp --query "[0].name" -o tsv)
WEB_HOST=$(az webapp show --resource-group rg-srelabapp --name "$WEB_APP" \
...
App Service Web App (with UAMI assigned)
  ↓
Microsoft.Data.SqlClient (Active Directory Managed Identity auth)
  ↓
Azure AD (issues AAD token for the UAMI)
  ↓
Azure SQL (validates token → contained user "srelabapp-id" with db_datareader)
  ↓
Database Access ✓
```
...
az webapp log tail --resource-group rg-srelabapp --name "$WEB_APP"
```
...
- The SQL connection string (carrying the UAMI client id) isn't set in the app settings — verify with `az webapp config appsettings list --resource-group rg-srelabapp --name "$WEB_APP" --query "[?name=='AZURE_SQL_CONNECTIONSTRING']"`
...
az webapp show --resource-group rg-srelabapp --name "$WEB_APP" --query "state"
```
...
az webapp restart --resource-group rg-srelabapp --name "$WEB_APP"
```
